...
The built-in legacy deep-link experiment records `app:feature_used` with the bounded `experimental` / `legacy_deeplinks` identifiers when a user enables, disables, or uses it. No URL, path, or query value is captured.
