...
private struct OnboardingWindowGlow: View {
    @Environment(\.accessibilityReduceMotion) private var reduceMotion
    @Environment(\.controlActiveState) private var controlActiveState

    var body: some View {
        RetainedOnboardingWindowGlow(
            reduceMotion: reduceMotion,
            isActive: controlActiveState != .inactive
        )
        .ignoresSafeArea()
...
private struct RetainedOnboardingWindowGlow: NSViewRepresentable {
    let reduceMotion: Bool
    let isActive: Bool

    func makeNSView(context: Context) -> RetainedOnboardingWindowGlowView {
        RetainedOnboardingWindowGlowView()
    }

    func updateNSView(_ nsView: RetainedOnboardingWindowGlowView, context: Context) {
        nsView.update(reduceMotion: reduceMotion, isActive: isActive)
    }
}

/// Keeps the onboarding perimeter animation on the compositor. The bloom is
/// clipped to the content bounds so it never creates a screen-sized transparent
/// surface behind the window.
@MainActor
private final class RetainedOnboardingWindowGlowView: NSView {
    private let ambientContainer = CALayer()
    private let ambientGradient = CAGradientLayer()
    private let ambientMask = CAShapeLayer()
    private let crispContainer = CALayer()
    private let crispGradient = CAGradientLayer()
    private let crispMask = CAShapeLayer()
    private let ambientBlur = CIFilter.gaussianBlur()
    ...
    override init(frame frameRect: NSRect) {
        ...
        let colors = [
            NSColor(srgbRed: 0.20, green: 0.82, blue: 0.98, alpha: 1).cgColor,
            NSColor(srgbRed: 0.28, green: 0.43, blue: 1.00, alpha: 1).cgColor,
            NSColor(srgbRed: 0.68, green: 0.25, blue: 0.98, alpha: 1).cgColor,
            NSColor(srgbRed: 0.98, green: 0.24, blue: 0.62, alpha: 1).cgColor,
            NSColor(srgbRed: 1.00, green: 0.55, blue: 0.16, alpha: 1).cgColor,
            NSColor(srgbRed: 0.20, green: 0.82, blue: 0.98, alpha: 1).cgColor
        ]

        for gradient in [ambientGradient, crispGradient] {
            gradient.type = .conic
            gradient.colors = colors
            gradient.locations = [0, 0.20, 0.40, 0.60, 0.80, 1]
            gradient.startPoint = CGPoint(x: 0.5, y: 0.5)
            gradient.endPoint = CGPoint(x: 0.5, y: 0)
        }

        ambientBlur.name = "onboardingWindowGlowBlur"
        ambientBlur.radius = 12
        ambientContainer.filters = [ambientBlur]
        ambientContainer.opacity = 0.48
        ambientContainer.mask = ambientMask
        ambientContainer.addSublayer(ambientGradient)

        crispContainer.opacity = 0.82
        crispContainer.mask = crispMask
        crispContainer.addSublayer(crispGradient)

        layer?.addSublayer(ambientContainer)
        layer?.addSublayer(crispContainer)
    }
    ...
    override func layout() {
        ...
        CATransaction.begin()
        CATransaction.setDisableActions(true)
        ambientContainer.frame = bounds
        crispContainer.frame = bounds
        ambientGradient.frame = gradientFrame
        crispGradient.frame = gradientFrame
        ambientMask.frame = bounds
        ambientMask.path = ambientPath
        ambientMask.fillColor = nil
        ambientMask.strokeColor = NSColor.white.cgColor
        ambientMask.lineWidth = 14
        crispMask.frame = bounds
        crispMask.path = crispPath
        crispMask.fillColor = nil
        crispMask.strokeColor = NSColor.white.cgColor
        crispMask.lineWidth = 2.5
        CATransaction.commit()
    }

    func update(reduceMotion: Bool, isActive: Bool) {
        guard lastReduceMotion != reduceMotion || lastIsActive != isActive else { return }
        lastReduceMotion = reduceMotion
        lastIsActive = isActive
    ...
    private func startAnimatingIfNeeded() {
        ...
        let baseAngle = CGFloat.pi / 5
        CATransaction.begin()
        CATransaction.setDisableActions(true)
        ambientGradient.setAffineTransform(CGAffineTransform(rotationAngle: baseAngle))
        crispGradient.setAffineTransform(CGAffineTransform(rotationAngle: baseAngle))
        ambientContainer.opacity = 0.48
        crispContainer.opacity = 0.82
        CATransaction.commit()

        let rotation = CABasicAnimation(keyPath: "transform.rotation.z")
        rotation.fromValue = baseAngle
        rotation.toValue = baseAngle + CGFloat.pi * 2
        rotation.duration = 11
        rotation.repeatCount = .infinity
        rotation.timingFunction = CAMediaTimingFunction(name: .linear)

        ambientGradient.add(rotation, forKey: "onboardingWindowGlowRotation")
        crispGradient.add(rotation, forKey: "onboardingWindowGlowRotation")

        let pulse = CABasicAnimation(keyPath: "opacity")
        pulse.fromValue = 0.34
        pulse.toValue = 0.58
        pulse.duration = 2.8
        pulse.autoreverses = true
        pulse.repeatCount = .infinity
        pulse.timingFunction = CAMediaTimingFunction(name: .easeInEaseOut)
        ambientContainer.add(pulse, forKey: "onboardingWindowGlowPulse")
    }
    ...
    private func stopAnimating() {
        ...
        resumeIfNeeded()
        isAnimating = false
        ambientGradient.removeAnimation(forKey: "onboardingWindowGlowRotation")
        crispGradient.removeAnimation(forKey: "onboardingWindowGlowRotation")
        ambientContainer.removeAnimation(forKey: "onboardingWindowGlowPulse")
        setStaticAppearance()
    }

    private func setStaticAppearance() {
        CATransaction.begin()
        CATransaction.setDisableActions(true)
        ambientGradient.setAffineTransform(CGAffineTransform(rotationAngle: .pi / 5))
        crispGradient.setAffineTransform(CGAffineTransform(rotationAngle: .pi / 5))
        ambientContainer.opacity = 0.38
        crispContainer.opacity = 0.72
        CATransaction.commit()
    }
